...
# 可选 OPENAI_BASE_URL 指定第三方 OpenAI 兼容网关（如 https://xxx/v1）：
# OPENAI_BASE_URL=https://your-gateway/v1 OPENAI_MODEL=gpt-4o OPENAI_API_KEY=sk-... npm run predict
```
...
1. **Settings → Secrets and variables → Actions** 添加两个密钥：
   ...
   - （可选）仓库变量 `OPENAI_BASE_URL` 指定第三方 OpenAI 兼容网关地址（如 `https://xxx/v1`）
